...
const db = new sqlite3.Database(dbPath, (err) => {
    if (err) console.error("Error connecting to database:", err);
        ...
        db.run(`INSERT INTO rooms (type, name, desc, price, priceWeekend, amenities, imgs) 
                ...
                WHERE NOT EXISTS (SELECT 1 FROM rooms WHERE type='sauna')`, (err) => {
            if (err) console.error("Error initializing Sauna:", err);
            else console.log("Sauna initialization checked/done.");
        });
    }
});
...
app.post('/api/bookings', (req, res) => {
    ...
    db.get(
        ...
        (err, row) => {
            ...
            db.run(
                `INSERT INTO bookings (id, type, room, checkIn, checkOut, nights, guest, phone, addons, total, status, createdAt)
                 VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
                [id, b.type, b.room, b.checkIn, b.checkOut, b.nights, guestName, guestPhone, JSON.stringify(b.addons || []), b.total, 'new', createdAt],
                function(err) {
                    if (err) return res.status(500).json({ error: err.message });
